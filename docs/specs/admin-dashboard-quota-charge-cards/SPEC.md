...
- `今日` 总览块继续使用小时请求窗口渲染累计用量背景图，当前窗口与昨日同刻窗口叠加显示；`本月` 总览块改为消费 overview / snapshot 的专用月度日粒度序列，并叠加 `previous-month comparison line`。两者都必须铺满整块卡片背景且不参与交互；未来日期保持 `null` 空槽，不伪造月内历史逐日形状。
