...
- `docs/specs/admin-recent-requests-unification/SPEC.md` 已把 `/admin/requests`、`/admin/keys/:id`、`/admin/tokens/:id` 的近期请求面板统一成共享组件，但 admin 侧仍保留了过时的“最多 200 条 / 最多 10 页”文案，和当前按 retention 保留的真实行为不一致。
