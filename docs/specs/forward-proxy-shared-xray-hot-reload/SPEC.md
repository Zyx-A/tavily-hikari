...
- 对 serving 角色，`/health` 必须直接反映 shared xray 的真实 readiness：如果当前 forward proxy 节点需要 local relay 但 shared xray 未就绪，立即返回 `503 xray not ready`，响应不得包含订阅 URL、节点 URL 或 key。`standby` / `recovery` 的 HA 最小健康豁免继续由 `f36b4` 管理。
...
- 对 serving 角色，`/health` 的 xray readiness 只在存在 local relay 依赖时生效；没有 local relay 依赖时仍返回 `200 ok`。forward-proxy 内部 startup grace 可以保留给非 `/health` 的内部 readiness 调用，但不得再决定 serving `/health`。
...
- 对 serving 角色，如果存在 xray relay 依赖但 shared xray 未就绪，`/health` 立即返回 `503 xray not ready`；shared xray 就绪后返回 `200 ok`。`standby` / `recovery` 不因为未预热 xray/runtime 而失败。
...
- `cargo test -q serving_health_ignores_startup_grace_and_requires_xray_readiness -- --nocapture`
