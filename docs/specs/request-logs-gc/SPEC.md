...
创建 `docs/specs/request-logs-gc/contracts/`，并只链接本计划需要的契约文件：
...
- `docs/specs/README.md`: 维护本主题的 lifecycle、实现状态与 canonical 路径。
- `docs/specs/request-logs-gc/contracts/*`: 作为接口契约的长期依据（实现阶段需保持一致）。
